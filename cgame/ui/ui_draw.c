...
void UI_DrawTextBox (float x, float y, float scale, int width, int lines)
{
	...
	cx = x;
	cy = y;
	cgi.R_DrawChar (NULL, cx, cy, scale, scale, FS_SHADOW, 1, Q_colorWhite);
	for (n=0 ; n<lines ; n++) {
		cy += (8*scale);
		cgi.R_DrawChar (NULL, cx, cy, scale, scale, FS_SHADOW, 4, Q_colorWhite);
	}
	cgi.R_DrawChar (NULL, cx, cy+(8*scale), scale, scale, FS_SHADOW, 7, Q_colorWhite);

	// draw middle
	cx += (8*scale);
	while (width > 0) {
		cy = y;
		cgi.R_DrawChar (NULL, cx, cy, scale, scale, FS_SHADOW, 2, Q_colorWhite);
		for (n = 0; n < lines; n++) {
			cy += (8*scale);
			cgi.R_DrawChar (NULL, cx, cy, scale, scale, FS_SHADOW, 5, Q_colorWhite);
		}
		cgi.R_DrawChar (NULL, cx, cy+(8*scale), scale, scale, FS_SHADOW, 8, Q_colorWhite);
		width -= 1;
		cx += (8*scale);
	}
	...
	// draw right side
	cy = y;
	cgi.R_DrawChar (NULL, cx, cy, scale, scale, FS_SHADOW, 3, Q_colorWhite);
	for (n=0 ; n<lines ; n++) {
		cy += (8*scale);
		cgi.R_DrawChar (NULL, cx, cy, scale, scale, FS_SHADOW, 6, Q_colorWhite);
	}
	cgi.R_DrawChar (NULL, cx, cy+(8*scale), scale, scale, FS_SHADOW, 9, Q_colorWhite);
}
...
static void Field_Draw (uiField_t *f)
{
	...
	if (f->generic.name)
		cgi.R_DrawString (NULL, x - ((Q_ColorCharCount (f->generic.name, (int)strlen (f->generic.name)) + 1) * UISIZE_TYPE (f->generic.flags)),
						y, UISCALE_TYPE (f->generic.flags), UISCALE_TYPE (f->generic.flags), txtflags, f->generic.name, Q_colorGreen);

	Q_strncpyz (tempbuffer, f->buffer + f->visibleOffset, f->visibleLength);

	// left
	cgi.R_DrawChar (NULL, x, y - (UISIZE_TYPE (f->generic.flags)*0.5), UISCALE_TYPE (f->generic.flags), UISCALE_TYPE (f->generic.flags), txtflags, 18, Q_colorWhite);
	cgi.R_DrawChar (NULL, x, y + (UISIZE_TYPE (f->generic.flags)*0.5), UISCALE_TYPE (f->generic.flags), UISCALE_TYPE (f->generic.flags), txtflags, 24, Q_colorWhite);

	// right
	cgi.R_DrawChar (NULL, x + UISIZE_TYPE (f->generic.flags) + (f->visibleLength * UISIZE_TYPE (f->generic.flags)), y - (UISIZE_TYPE (f->generic.flags)*0.5), UISCALE_TYPE (f->generic.flags), UISCALE_TYPE (f->generic.flags), txtflags, 20, Q_colorWhite);
	cgi.R_DrawChar (NULL, x + UISIZE_TYPE (f->generic.flags) + (f->visibleLength * UISIZE_TYPE (f->generic.flags)), y + (UISIZE_TYPE (f->generic.flags)*0.5), UISCALE_TYPE (f->generic.flags), UISCALE_TYPE (f->generic.flags), txtflags, 26, Q_colorWhite);

	// middle
	for (i=0 ; i<f->visibleLength ; i++) {
		cgi.R_DrawChar (NULL, x + UISIZE_TYPE (f->generic.flags) + (i * UISIZE_TYPE (f->generic.flags)), y - (UISIZE_TYPE (f->generic.flags)*0.5), UISCALE_TYPE (f->generic.flags), UISCALE_TYPE (f->generic.flags), txtflags, 19, Q_colorWhite);
		cgi.R_DrawChar (NULL, x + UISIZE_TYPE (f->generic.flags) + (i * UISIZE_TYPE (f->generic.flags)), y + (UISIZE_TYPE (f->generic.flags)*0.5), UISCALE_TYPE (f->generic.flags), UISCALE_TYPE (f->generic.flags), txtflags, 25, Q_colorWhite);
	}

	// text
	curOffset = cgi.R_DrawString (NULL, x + UISIZE_TYPE (f->generic.flags), y, UISCALE_TYPE (f->generic.flags), UISCALE_TYPE (f->generic.flags), txtflags, tempbuffer, Q_colorWhite);
	...
	if (UI_ItemAtCursor (f->generic.parent) == f) {
		...
		if ((((int)(cg.realTime / 250))&1))
			cgi.R_DrawChar (NULL, x + (offset * UISIZE_TYPE (f->generic.flags)),
				y, UISCALE_TYPE (f->generic.flags), UISCALE_TYPE (f->generic.flags), txtflags, 11, Q_colorWhite);
	}
}
...
static void Slider_Draw (uiSlider_t *s)
{
	...
	if (s->generic.name) {
		cgi.R_DrawString (NULL, s->generic.x + s->generic.parent->x + LCOLUMN_OFFSET - ((Q_ColorCharCount (s->generic.name, (int)strlen (s->generic.name)) - 1)*UISIZE_TYPE (s->generic.flags)),
			s->generic.y + s->generic.parent->y, UISCALE_TYPE (s->generic.flags), UISCALE_TYPE (s->generic.flags), txtflags, s->generic.name, Q_colorGreen);
	}

	s->range = clamp (((s->curValue - s->minValue) / (float) (s->maxValue - s->minValue)), 0, 1);

	// left
	cgi.R_DrawChar (NULL, s->generic.x + s->generic.parent->x + RCOLUMN_OFFSET, s->generic.y + s->generic.parent->y, UISCALE_TYPE (s->generic.flags), UISCALE_TYPE (s->generic.flags), txtflags, 128, Q_colorWhite);

	// center
	for (i=0 ; i<SLIDER_RANGE ; i++)
		cgi.R_DrawChar (NULL, RCOLUMN_OFFSET + s->generic.x + i*UISIZE_TYPE (s->generic.flags) + s->generic.parent->x + UISIZE_TYPE (s->generic.flags), s->generic.y + s->generic.parent->y, UISCALE_TYPE (s->generic.flags), UISCALE_TYPE (s->generic.flags), txtflags, 129, Q_colorWhite);

	// right
	cgi.R_DrawChar (NULL, RCOLUMN_OFFSET + s->generic.x + i*UISIZE_TYPE (s->generic.flags) + s->generic.parent->x + UISIZE_TYPE (s->generic.flags), s->generic.y + s->generic.parent->y, UISCALE_TYPE (s->generic.flags), UISCALE_TYPE (s->generic.flags), txtflags, 130, Q_colorWhite);

	// slider bar
	cgi.R_DrawChar (NULL, (UISIZE_TYPE (s->generic.flags) + RCOLUMN_OFFSET + s->generic.parent->x + s->generic.x + (SLIDER_RANGE-1)*UISIZE_TYPE (s->generic.flags) * s->range), s->generic.y + s->generic.parent->y, UISCALE_TYPE (s->generic.flags), UISCALE_TYPE (s->generic.flags), txtflags, 131, Q_colorWhite);
}
...
static void SpinControl_Draw (uiList_t *s)
{
	...
	if (!strchr (s->itemNames[s->curValue], '\n')) {
		cgi.R_DrawString (NULL, RCOLUMN_OFFSET + s->generic.x + s->generic.parent->x,
					s->generic.y + s->generic.parent->y, UISCALE_TYPE (s->generic.flags), UISCALE_TYPE (s->generic.flags), txtflags, s->itemNames[s->curValue], Q_colorWhite);
	}
	else {
		Q_strncpyz (buffer, s->itemNames[s->curValue], sizeof (buffer));
		*strchr (buffer, '\n') = 0;
		cgi.R_DrawString (NULL, RCOLUMN_OFFSET + s->generic.x + s->generic.parent->x,
					s->generic.y + s->generic.parent->y, UISCALE_TYPE (s->generic.flags), UISCALE_TYPE (s->generic.flags), txtflags, buffer, Q_colorWhite);

		Q_strncpyz (buffer, strchr (s->itemNames[s->curValue], '\n') + 1, sizeof (buffer));
		cgi.R_DrawString (NULL, RCOLUMN_OFFSET + s->generic.x + s->generic.parent->x,
					s->generic.y + s->generic.parent->y + UISIZEINC_TYPE(s->generic.flags), UISCALE_TYPE (s->generic.flags), UISCALE_TYPE (s->generic.flags), txtflags, buffer, Q_colorWhite);
	}
}
...
void UI_DrawInterface (uiFrameWork_t *fw)
{
	...
	if (item && item->cursorDraw)
		...
		if (item->flags & UIF_LEFT_JUSTIFY)
			cgi.R_DrawChar (NULL, fw->x + item->x + LCOLUMN_OFFSET + item->cursorOffset, fw->y + item->y, UISCALE_TYPE (item->flags), UISCALE_TYPE (item->flags), 0, 12 + ((int)(cg.realTime/250) & 1), Q_colorWhite);
		else if (item->flags & UIF_CENTERED)
			cgi.R_DrawChar (NULL, fw->x + item->x - (((Q_ColorCharCount (item->name, (int)strlen (item->name)) + 4) * UISIZE_TYPE (item->flags))*0.5) + item->cursorOffset, fw->y + item->y, UISCALE_TYPE (item->flags), UISCALE_TYPE (item->flags), 0, 12 + ((int)(cg.realTime/250) & 1), Q_colorWhite);
		else
			cgi.R_DrawChar (NULL, fw->x + item->x + item->cursorOffset, fw->y + item->y, UISCALE_TYPE (item->flags), UISCALE_TYPE (item->flags), 0, 12 + ((int)(cg.realTime/250) & 1), Q_colorWhite);
	}
	...
	if (selitem && (selitem->type != UITYPE_FIELD) && (selitem->type != UITYPE_IMAGE) && (!(selitem->flags & UIF_NOSELECT))) {
		if (selitem->flags & UIF_LEFT_JUSTIFY)
			cgi.R_DrawChar (NULL, fw->x + selitem->x + LCOLUMN_OFFSET + selitem->cursorOffset, fw->y + selitem->y, UISCALE_TYPE (selitem->flags), UISCALE_TYPE (selitem->flags), 0, 12 + ((int)(cg.realTime/250) & 1), Q_colorWhite);
		else if (selitem->flags & UIF_CENTERED)
			cgi.R_DrawChar (NULL, fw->x + selitem->x - (((Q_ColorCharCount (selitem->name, (int)strlen (selitem->name)) + 4) * UISIZE_TYPE (selitem->flags))*0.5) + selitem->cursorOffset, fw->y + selitem->y, UISCALE_TYPE (selitem->flags), UISCALE_TYPE (selitem->flags), 0, 12 + ((int)(cg.realTime/250) & 1), Q_colorWhite);
		else
			cgi.R_DrawChar (NULL, fw->x + selitem->x + selitem->cursorOffset, fw->y + selitem->y, UISCALE_TYPE (selitem->flags), UISCALE_TYPE (selitem->flags), 0, 12 + ((int)(cg.realTime/250) & 1), Q_colorWhite);
	}
...
void UI_Refresh (qBool fullScreen)
{
	...
	// Draw the backdrop
	if (fullScreen) {
		cgi.R_DrawPic (uiMedia.bgBig, 0, 0, 0, cg.refConfig.vidWidth, cg.refConfig.vidHeight, 0, 0, 1, 1, Q_colorWhite);
	}
	else {
