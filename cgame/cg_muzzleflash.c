...
static void CG_BrassHack (cgEntity_t *pl, leType_t type, int count)
{
	...
	for (i=0 ; i<count ; i++) {
		CG_SpawnLocalEnt(pl->current.origin[0] + forward[0] + right[0] + crand (),
						pl->current.origin[1] + forward[1] + right[1] + crand (),
						pl->current.origin[2] + forward[2] + right[2] + crand (),
						(forward[0] * ((frand () * 2.5f) + 0.5f)) + (crand () * 45.0f),
...
void CG_ParseMuzzleFlash (void)
{
	...
	case MZ_BLASTER:
		...
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.blasterFireSfx, volume, ATTN_NORM, 0);
		break;

	// MZ_BLUEHYPERBLASTER
	case MZ_BLUEHYPERBLASTER:
		Vec3Set (dl->color, 0, 0, 1);
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.hyperBlasterFireSfx, volume, ATTN_NORM, 0);
		break;
	...
	case MZ_SHOTGUN:
		Vec3Set (dl->color, 1, 1, 0);
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.shotgunFireSfx, volume, ATTN_NORM, 0);
		cgi.Snd_StartSound (NULL, entNum, CHAN_AUTO, cgMedia.sfx.mz.shotgunReloadSfx, volume, ATTN_NORM, 0.1f);
		break;

	// MZ_SSHOTGUN
	case MZ_SSHOTGUN:
		Vec3Set (dl->color, 1, 1, 0);
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.superShotgunFireSfx, volume, ATTN_NORM, 0);
		break;

	// MZ_CHAINGUN1
	case MZ_CHAINGUN1:
		Vec3Set (dl->color, 1, 0.25, 0);
	...
	case MZ_CHAINGUN3:
		...
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.machineGunSfx[(rand () % 5)], volume, ATTN_NORM, 0.066f);
		break;

	// MZ_RAILGUN
	case MZ_RAILGUN:
		Vec3Set (dl->color, 0.5, 0.5, 1);
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.railgunFireSfx, volume, ATTN_NORM, 0);
		break;
	...
	case MZ_ROCKET:
		...
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.rocketFireSfx, volume, ATTN_NORM, 0);
		cgi.Snd_StartSound (NULL, entNum, CHAN_AUTO, cgMedia.sfx.mz.rocketReloadSfx, volume, ATTN_NORM, 0.1f);
		break;

	// MZ_GRENADE
	case MZ_GRENADE:
		Vec3Set (dl->color, 1, 0.5f, 0);
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.grenadeFireSfx, volume, ATTN_NORM, 0);
		cgi.Snd_StartSound (NULL, entNum, CHAN_AUTO, cgMedia.sfx.mz.grenadeReloadSfx, volume, ATTN_NORM, 0.1f);
		break;

	// MZ_BFG
	case MZ_BFG:
		Vec3Set (dl->color, 0, 1, 0);
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.bfgFireSfx, volume, ATTN_NORM, 0);
		break;
	...
	case MZ_PHALANX:
		...
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.phalanxFireSfx, volume, ATTN_NORM, 0);
		break;
	// RAFAEL
	...
	case MZ_IONRIPPER:
		Vec3Set (dl->color, 1, 0.5, 0.5);
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.ionRipperFireSfx, volume, ATTN_NORM, 0);
		break;
	...
	case MZ_ETF_RIFLE:
		...
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.etfRifleFireSfx, volume, ATTN_NORM, 0);
		break;

	// MZ_SHOTGUN2
	case MZ_SHOTGUN2:
		Vec3Set (dl->color, 1, 1, 0);
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.shotgun2FireSfx, volume, ATTN_NORM, 0);
		break;

	// MZ_HEATBEAM
	case MZ_HEATBEAM:
		Vec3Set (dl->color, 1, 1, 0);
		dl->die = cg.realTime + 100;
		break;
	...
	case MZ_TRACKER:
		...
		cgi.Snd_StartSound (NULL, entNum, CHAN_WEAPON, cgMedia.sfx.mz.trackerFireSfx, volume, ATTN_NORM, 0);
		break;
