...
uniform mat4 u_NormalMatrix;  // Optional precomputed normal matrix (upper-left 3x3 used)
uniform float u_UseNormalMatrix; // 0.0 = compute in shader, 1.0 = use u_NormalMatrix
...
void main()
{
    ...
    // Transform normal by inverse-transpose to handle non-uniform scale
    // Fast path: allow CPU to provide u_NormalMatrix for rigid transforms
    mat3 normalMatrix = (u_UseNormalMatrix > 0.5)
        ? mat3(u_NormalMatrix)
        : transpose(inverse(mat3(u_Model)));
