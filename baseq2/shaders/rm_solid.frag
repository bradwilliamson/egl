...
void main()
{
    ...
    if (u_UseLightmap > 0.5) {
        // Lightmap path: sample full RGB and modulate
        vec3 lightSample = texture(u_Lightmap, v_LmCoord).rgb;
        vec3 lightMod = lightSample * 2.0;  // Standard Quake 2 modulation
        finalColor = texColor.rgb * lightMod;
    } else {
