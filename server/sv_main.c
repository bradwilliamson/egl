...
static void SV_FlushRedirect (int target, char *outBuffer)
{
	...
	case RD_CLIENT:
		...
		MSG_WriteString (&sv_currentClient->netChan.message, outBuffer);
		break;
	}
}
...
=================
*/
static void SVC_GetChallenge_f (void)
{
	int		i;
	int		oldest;
	uint32	oldestTime;
...
==================
*/
static void SVC_DirectConnect_f (void)
{
	...
	for (i=0, cl=svs.clients ; i<maxclients->intVal ; i++, cl++) {
		...
		&& (cl->netChan.qPort == qPort || adr.port == cl->netChan.remoteAddress.port)) {
			if (!NET_IsLocalAddress (adr) && svs.realTime-cl->lastConnect < sv_reconnect_limit->intVal*1000) {
...
===============
*/
static void SVC_RemoteCommand_f (void)
{
	char	remaining[1024];
	int		i;
...
static void SV_ConnectionlessPacket (void)
{
	...
	assert (MSG_ReadLong (&sv_netMessage) == -1);		// skip the -1 marker
...
void SV_Frame (int msec)
{
	...
	if (!sv_timedemo->floatVal && (uint32)svs.realTime < sv.time) {
...
==================
*/
static void SV_FinalMessage (char *message, qBool reconnect)
{
	int			i;
	svClient_t	*cl;
...
================
*/
void SV_ServerShutdown (char *finalMessage, qBool reconnect, qBool crashing)
{
	if (svs.clients)
