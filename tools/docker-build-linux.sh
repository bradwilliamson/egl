...
# Build client + modules + dedicated using Makefile.unix inside a Linux container.
DOCKER_CMD="docker"
if [ -n "${MSYSTEM:-}" ] || [ -n "${MSYS:-}" ]; then
	# When invoked via msys sh from PowerShell, docker.exe argument handling can get mangled.
	# Going through cmd.exe avoids MSYS argument conversion entirely.
