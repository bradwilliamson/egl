...
# Run the Linux packaging (tools/package.sh) inside a Linux container.
DOCKER_CMD="docker"
if [ -n "${MSYSTEM:-}" ] || [ -n "${MSYS:-}" ]; then
...
$DOCKER_CMD run --rm -t \
  ...
  -w /src \
  "${IMAGE}" \
  sh -lc "apt-get update && DEBIAN_FRONTEND=noninteractive apt-get install -y build-essential pkg-config ca-certificates libcurl4-openssl-dev libsdl2-dev libxrandr-dev libx11-dev libxi-dev libgl1-mesa-dev libjpeg-dev libpng-dev libminizip-dev zip && bash tools/package.sh"
